<script>
    ...
    function remainingChildren(data, child) {
        ...
        console.log(`remaining of ${data.length} ${child.id}`, remaining);
        return remaining;
    }
...
{#if currentItem.children.length > 0}
    <tr class="{depth != 0 ? 'greyed' : ''} arrow" on:click={() => toggle()}>
        ...
        <td class="colored" style="--rowcolor: {currentItem.cpu_time / max}">
            {format(currentItem.cpu_time)} - {Math.floor(
                (currentItem.cpu_time / max) * 100,
            )}%
        </td>
    </tr>
{:else}
    <tr class={depth != 0 ? "greyed" : ""}>
        <td>
            <div
                ...
                style="--padding: calc({depth * 30}px);"
            >
                {currentItem.id}
            </div>
        </td>
